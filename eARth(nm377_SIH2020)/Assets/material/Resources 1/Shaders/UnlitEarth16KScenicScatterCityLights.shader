	...
	SubShader 
	{
    	...
    	Pass 
    	{
			...
			v2f vert(appdata_tan v)
			{
				...
				float3 v3Attenuate;
//				for(int i=0; i<int(fSamples); i++)	// manually unrolled
				{
			...
			half4 frag(v2f IN) : SV_Target
			{
				...
				if (IN.uv.x<0.5) {
					if (IN.uv.y>0.5) {
						color = tex2D (_TexTL, IN.uv.xy * 2.0.xx).rgb;
					} else {
						color = tex2D (_TexBL, half2(IN.uv.x * 2.0f, (IN.uv.y-0.5) * 2.0)).rgb;	
					}
				} else {
					if (IN.uv.y>0.5) {
						color = tex2D (_TexTR, half2((IN.uv.x - 0.5) * 2.0f, IN.uv.y * 2.0)).rgb;
					} else {
						color = tex2D (_TexBR, half2((IN.uv.x - 0.5) * 2.0f, (IN.uv.y-0.5) * 2.0)).rgb;	
					}
				}
				half3 cityLights = tex2D(_CityLights, IN.uv).rgb;
				...
				// apply clouds
				earth += (cloud * 1.5 - clamp(shadows, shadows, 1-cloud)) * _CloudAlpha;
				...
				earth *= col.b;
				earth += cityLights * (1.0 - saturate(col.g * 4));
//				return half4(earth*1.35+col*0.65,1.0);
				color = earth + col * 0.65;
				
				// apply final contrast & brightness
		  		color = (color - 0.5.xxx) * _Contrast + 0.5.xxx;
				color *= _Brightness;
